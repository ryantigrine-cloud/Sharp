...
Pointer-down on a form button preserves the focused answer field. Native click, form validation and submit still do the work, so submitting does not require first closing the keyboard. Dragging away does not submit. Existing answer/timeout settlement guards are unchanged.
