...
function sessionFrame(content, timer = true) {
  return `<main class="app"><section class="session">${headProgress(timer)}${content}</section></main>`;
}
function questionScreen() {
  ...
  root.innerHTML = sessionFrame(`<div class="qwrap"><div class="eye">${META[active.id][0]} · Level ${active.level} · ${active.qi + 1}/${active.count}</div>${content}<p class="sub">${esc(q.sub || '')}</p></div>${encoding ? '' : answerUI(q)}`, !encoding);
  if (!encoding) focusInput();
}
function dualScreen() {
  const encoding = active.phase === 'dualEncoding';
  root.innerHTML = sessionFrame(`<div class="qwrap"><div class="eye">Dual Task · Level ${active.level}</div>${encoding ? `<h2>Memorise and hold</h2><div class="q">${active.dualSeq.join('   ')}</div><p>Recall it backwards after the arithmetic.</p>` : `<div class="q compact">${esc(active.q.prompt)}</div><p>${active.phase === 'dualMath' ? 'Solve this while holding the sequence.' : 'Enter the original sequence backwards.'}</p>`}</div>${encoding ? '' : numberForm(active.q, 'dualForm')}`, !encoding);
  if (!encoding) focusInput();
}
