...
function updateViewport() {
  ...
  // Do not resize or chase the user's viewport while they pinch-zoom/pan.
  if (vv && Math.abs(vv.scale - 1) > .05) return;
  const height = vv?.height || window.innerHeight;
  const doc = document.documentElement;
  doc.style.setProperty('--usable-height', `${height}px`);
  doc.style.setProperty('--viewport-top', `${vv?.offsetTop || 0}px`);
  const layoutHeight = Math.max(window.innerHeight, doc.clientHeight || 0);
  // Geometry, not focusout: a submit tap must not expand the layout before click.
  doc.classList.toggle('keyboard-open', height < layoutHeight - 100);
}
